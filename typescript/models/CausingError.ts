...
export interface CausingError {
     ...
     * * `MATRIX_ROUTING_UNSUPPORTED_CURRENCY` - The specified currency is not supported.
     *   * `value` - The unsupported currency.
     ...
     * * `MATRIX_ROUTING_LOCATIONS_TOO_FAR_AWAY` - Locations are too far away from each other.
     *   * `limit`- All locations have to be inside a rectangle with edges of at most 500 km length.
     * * `MATRIX_ROUTING_UNSUPPORTED_RESULT_OPTION` - The specified result option is not supported.
     *   * `value` - The unsupported result option.
     * * `MATRIX_ROUTING_UNSUPPORTED_PLAIN_RESULT_FORMAT_WITH_TRAVEL_TIME_PROFILES` - The travel time profiles cannot be requested in combination with the result format PLAIN.
     * * `MATRIX_ROUTING_UNSUPPORTED_OPTIONS_WITH_SYNC` - This option is not supported with synchronous call.
     * * `MATRIX_ROUTING_TRAVEL_TIMES_MUST_BE_REQUESTED` - The results should contain TRAVEL_TIMES.
     ...
     * * `ROUTE_ID_ESTIMATED_DISTANCE_TOO_LONG` - The distance of the route (estimated by air-line) for the selected vehicle is too long. - _The **parameter** remains empty._
     ...
     * 
     * **Error codes for** `MATRIX_ROUTING_CONTENT_TOO_LARGE`
     * 
     * * `MATRIX_ROUTING_DECOMPRESSED_SIZE_EXCEEDED` - The decompressed size of the body exceeds the limit.
     *   * `limit`- The decompressed body size limit [bytes].
     * 
     * **Error codes for** `ROUTE_ID_ERROR`
     * 
     * * `ROUTE_ID_LOCATION_CANNOT_BE_MATCHED` - The location cannot be matched to the nearest possible road.
     * * `ROUTE_ID_ROUTE_NOT_FOUND` - A route between at least two locations could not be found for the current configuration and profile.
     * @type {string}
     * @memberof CausingError
     */
    errorCode: string;
